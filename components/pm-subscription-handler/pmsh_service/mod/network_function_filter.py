...
def filter_diff(app_conf, existing_subscription_filter):
    """Check for a difference in new and existing filter

    Args:
        app_conf (AppConfig): the application configuration.
        existing_subscription_filter (NetworkFunctionFilter): Existing filter on subscription

    Returns:
        bool: True for a difference in filter
    """
...
def process_nfs_for_creation_and_deletion(existing_nfs, new_nfs, mrpub, app_conf):
    """Finds nfs for deletion/creation and processes the action

    Args:
        existing_nfs(list[NetworkFunction]): List of existing nfs on subscription.
        new_nfs(list[NetworkFunction]): List of new nfs from new filter.
