...
def add_new_filtered_nfs(filtered_nfs, unlocked_mgs, sub_model):
    ...
    Args:
        filtered_nfs (List[NetworkFunctionModel]): nfs to be inserted
...
def update_filter(sub_name, nf_filter):
    ...
    save_nf_filter_update(sub_name, nf_filter)
    ...
    if unlocked_mgs:
        ...
        remove_un_filtered_nfs(del_nfs, sub_model, unlocked_mgs)
    db.session.remove()
...
def remove_un_filtered_nfs(del_nfs, sub_model, unlocked_mgs):
    """
...
def extract_del_new_nfs(sub_model):
    ...
    Returns:
        del_nfs (List[String]): Names of nfs to be deleted
        new_nfs (List[NetworkFunctionModel]): nfs to be inserted
    """
    filtered_nfs = nf_service.capture_filtered_nfs(sub_model.subscription_name)
