...
class Subscription:
    ...
    def _filter_diff(self, existing_subscription_filter):
        """
        Check if there is a difference between the new and old filter.

        Args:
            existing_subscription_filter: the current NF filter.

        Returns:
            Boolean: True if the filters are different, otherwise false.
    ...
    def update_subscription_filter(self):
        ...
        Raises: Exception when status update fails
        """
        try:
            SubscriptionModel.query.filter(
                SubscriptionModel.subscription_name == self.subscriptionName) \
    ...
    def get_nfs_for_creation_and_deletion(nfs1, nfs2, action, mrpub, app_conf):
        """ Finds NFs to be updated or deleted

        Args:
            nfs1 (str): List of nfs
            nfs2 (str): List of nfs
