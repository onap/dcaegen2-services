...
class Subscription:
            ...
            if existing_subscription is None:
                ...
                if existing_subscription.nfFilter and \
                    ...
                    self.app_conf.subscription.administrativeState = \
                        AdministrativeState.FILTERING.value
                    self.app_conf.subscription.nfFilter = existing_subscription.nfFilter
    ...
    def get_nfs_for_creation_and_deletion(self, nfs1, nfs2, action):
        for x in range(len(nfs1)):
            found = 0
            for y in range(len(nfs2)):
                if (nfs1[x]['properties']['pnf-name'] == nfs2[y]['properties']['pnf-name']):
                    found = 1
                if found == 0 and action == 'create':
                    self.create_subscription_on_nfs(nfs1[x], self.mrpub, self.app_conf)
                elif found == 0 and action == 'delete':
                    self.delete_subscription_from_nfs(nfs1[x], self.mrpub, self.app_conf)
