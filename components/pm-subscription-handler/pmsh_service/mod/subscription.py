...
def get_nfs_for_creation_and_deletion(existing_nfs, new_nfs, action, mrpub, app_conf):
    for x in range(len(existing_nfs)):
        found = 0
        for y in range(len(new_nfs)):
            if existing_nfs[x].nf_name == new_nfs[y].nf_name:
                found = 1
        if found == 0 and action == 'create':
            app_conf.subscription.create_subscription_on_nfs([new_nfs[x]], mrpub, app_conf)
        elif found == 0 and action == 'delete':
            app_conf.subscription.delete_subscription_from_nfs([existing_nfs[x]], mrpub, app_conf)
