...
def audit(raw_request, bts, xer, rcode, calling_mod, msg="n/a"):
    """
    write an EELF audit record per
    https://wiki.onap.org/download/attachments/1015849/ONAP%20application%20
    logging%20guidelines.pdf?api=v2
    ...
    _AUDIT_LOGGER.info(
        "{bts}|{ets}|{xer}||n/a||{path}||{status}|"
        "{rcode}|||INFO||{servip}|{et}|config_binding_service|"
        "{clientip}|{calling_mod}|||||||{msg}".format(
            bts=bts.isoformat(),
            ets=ets.isoformat(),
            xer=xer,
            rcode=rcode,
...
def error(raw_request, xer, severity, ecode, tgt_entity="n/a",
    ...
    """
    write an EELF error record per 'https://wiki.onap.org/download/
    attachments/1015849/ONAP%20application%20logging%20guidelines.pdf?api=v2'
...
def metrics(raw_request, bts, xer, target, target_path, rcode,
            calling_mod, msg="n/a"):
    """
    write an EELF metrics record per 'https://wiki.onap.org/download/
    attachments/1015849/ONAP%20application%20logging%20guidelines.pdf?api=v2'
    ...
    _METRICS_LOGGER.info(
        "{bts}|{ets}|{xer}||n/a||{path}||{tge}|{tgp}|"
        "{status}|{rcode}|||INFO||{servip}|{et}|config_binding_service|"
        "{clientip}|{calling_mod}|||n/a|||||{msg}"
        .format(
            bts=bts.isoformat(),
            ets=ets.isoformat(),
            xer=xer,
