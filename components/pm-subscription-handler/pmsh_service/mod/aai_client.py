    ...
    try:
        for xnf in xnf_data['results']:
            if xnf['node-type'] == 'pnf':
                if sub.apply_filter_to_xnf(xnf['properties']['pnf-name']):
                ...
                if sub.apply_filter_to_xnf(xnf['properties']['vnf-name']):
...
class Xnf:
    ...
    def __str__(self):
        return f'xnf-name: {self.xnf_name}, ' \
               f'orchestration-status: {self.orchestration_status}'
