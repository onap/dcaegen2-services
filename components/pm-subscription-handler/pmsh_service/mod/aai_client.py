    ...
    try:
        for xnf in xnf_data['results']:
            if xnf['node-type'] == 'pnf':
                if xnf_filter.is_xnf_in_filter(xnf['properties'].get('pnf-name')):
                    pnf_obj = Xnf(xnf_name=xnf['properties'].get('pnf-name'),
                                  orchestration_status=xnf['properties'].get(
                                      'orchestration-status'))
                    xnf_set.add(pnf_obj)
            else:
                if xnf_filter.is_xnf_in_filter(xnf['properties'].get('vnf-name')):
                    vnf_obj = Xnf(xnf_name=xnf['properties'].get('vnf-name'),
                                  orchestration_status=xnf['properties'].get(
                                      'orchestration-status'))
                    xnf_set.add(vnf_obj)
    except KeyError as e:
