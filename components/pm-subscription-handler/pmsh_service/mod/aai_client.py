...
def _filter_nf_data(nf_data, nf_filter):
    ...
    try:
        for nf in nf_data['results']:
            ...
            invariant_uuid = nf['properties'].get('model-invariant-id')
            uuid = nf['properties'].get('model-version-id')
            if nf_filter.is_nf_in_filter(nf['properties'].get(name_identifier), invariant_uuid, uuid, orchestration_status):
