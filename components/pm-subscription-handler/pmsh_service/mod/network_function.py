...
class NetworkFunctionFilter:
    ...
    def is_nf_in_filter(self, nf_name, model_invariant_id, model_version_id, orchestration_status):
        ...
        if self.nf_names and self.regex_matcher.search(nf_name) is None:
            match = False
        if self.model_invariant_ids and not len([entityid for entityid in self.model_invariant_ids
                                             if entityid == model_invariant_id]) > 0:
            match = False
        if self.model_version_ids and not len([entityid for entityid in self.model_version_ids if entityid == model_version_id]) > 0:
            match = False
        return match
