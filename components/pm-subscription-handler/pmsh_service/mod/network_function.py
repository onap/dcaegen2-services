...
class NetworkFunctionFilter:
    ...
    def is_nf_in_filter(self, nf_name, invariant_uuid, uuid, orchestration_status):
        ...
        """
        # aaistatus = True
        # if self.nf_names is not None:
        #     if self.regex_matcher.search(nf_name) is None:
        #         aaistatus = False
        #
        # if self.invariant_uuids is not None and len(self.invariant_uuids) != 0 and aaistatus is not False:
        #     aaistatus = self.check_filter_item(self.invariant_uuids, invariant_uuid)
        #
        # if self.uuids is not None and len(self.uuids) != 0 and aaistatus is not False:
        #     aaistatus = self.check_filter_item(self.uuids, uuid)
        #
        # return aaistatus and \
    ...
    # @staticmethod
    # def check_filter_item(ids, aaientity):
    #     status = False
    #     for entityid in ids:
    #         if aaientity == entityid:
    #             status = True
    #     return status
