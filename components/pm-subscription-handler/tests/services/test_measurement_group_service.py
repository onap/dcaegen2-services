...
class MeasurementGroupServiceTestCase(BaseClassSetup):
    ...
    @patch.object(AppConfig, 'publish_to_topic')
    def test_update_admin_status_to_unlocking(self, mock_mr):
        super().setUpAppConf()
        sub = create_subscription_data('sub')
        db.session.add(sub)
        db.session.commit()
    ...
    def test_update_admin_status_for_missing_measurement_group(self):
        try:
            ...
            self.assertEqual(e.invalid_message, 'Requested measurement group not available '
    ...
    def test_update_admin_status_for_data_conflict(self):
        ...
        try:
            ...
            self.assertEqual(e.data_conflict_message, 'Cannot update admin status as Locked request'
    ...
    def test_update_admin_status_for_same_state(self):
        ...
        try:
            ...
            self.assertEqual(e.invalid_message, 'Measurement group is already in UNLOCKED '
                                                'state for sub name: sub1  and meas group '
                                                'name: MG1')

    def test_delete_nf_to_measurement_group_for_locking_with_LOCKED_update(self):
        sub = create_subscription_data('sub')
        ...
        db.session.commit()
        measurement_group_service.delete_nf_to_meas_grp_for_locking(
    ...
    def test_delete_nf_to_measurement_group_for_locking_with_no_LOCKED_update(self):
        sub = create_subscription_data('sub')
        sub.measurement_groups[1].administrative_state = 'LOCKING'
        nf_list = create_multiple_network_function_data(['pnf_101', 'pnf_102'])
        db.session.add(sub)
