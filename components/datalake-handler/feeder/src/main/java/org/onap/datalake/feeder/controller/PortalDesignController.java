...
public class PortalDesignController {
	...
	@PostMapping("")
	@ResponseBody
	@ApiOperation(value="CreateOrUpdate a portalDesign.")
    public PostReturnBody<PortalDesignConfig> createOrUpdate(@RequestBody PortalDesignConfig portalDesignConfig, BindingResult result, HttpServletResponse response) throws IOException {
		...
		if (portalDesignConfig.getId() == null) {
			...
			//update
			portalDesign = portalDesignService.getPortalDesign(portalDesignConfig.getId());
			if (portalDesign != null) {
				...
				return mkPostReturnBody(400, portalDesign);
			}
	...
	private PostReturnBody<PortalDesignConfig> mkPostReturnBody(int statusCode, PortalDesign portalDesign)
	{
		...
		PortalDesignConfig portalDesignConfig = null;
		try{
			portalDesignConfig = portalDesign.getPortalDesignConfig();
		} catch (Exception e){
			portalDesignConfig = new PortalDesignConfig();
		}
        retBody.setStatusCode(statusCode);
        retBody.setReturnBody(portalDesignConfig);
