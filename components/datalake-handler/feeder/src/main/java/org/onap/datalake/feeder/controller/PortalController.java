...
public class PortalController {
    ...
    public PostReturnBody<PortalConfig> updatePortal(@RequestBody PortalConfig portalConfig, BindingResult result, HttpServletResponse response) throws IOException {
        ...
        if (result.hasErrors()) {
            sendError(response, 400, "Error update Portal: "+result.toString());
            return null;
        }
    ...
    public List<PortalConfig> getPortalsEnable(@PathVariable Boolean enabled) {
    ...
    @GetMapping("/getNames/{enabled}")
    @ResponseBody
    @ApiOperation(value = "All portals names disable")
    public List<String> getPortalNamesDisable(@PathVariable boolean enabled) {
        return portalService.listNamesDisable(enabled);
    }
