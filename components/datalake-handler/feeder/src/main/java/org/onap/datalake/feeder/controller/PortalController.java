...
public class PortalController {
    ...
    @PutMapping("")
    @ResponseBody
    @ApiOperation("Create or update portal")
    public PostReturnBody<PortalConfig> createOrUpdate(@RequestBody PortalConfig portalConfig, BindingResult result, HttpServletResponse response) throws IOException {

        if (result.hasErrors()) {
            sendError(response, 400, "Error createOrUpdate Portal: "+result.toString());
            return null;
        } else {

            Portal portal = portalService.fillPortalConfiguration(portalConfig);
            String name = portal.getName();
            portal.setDb(portalRepository.findById(name).get().getDb());
    ...
    public void deletePortal(@PathVariable String name, HttpServletResponse response) throws IOException{

        Portal portal = portalRepository.findById(name).get();
        if (portal == null) {
            sendError(response, 400, "Error delete Portal: "+name);
        } else {
