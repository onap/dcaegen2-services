...
CREATE TABLE `dashboard_template` (
  ...
  CONSTRAINT `FK_type` FOREIGN KEY (`type`) REFERENCES `dashboards` (`name`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `dashboard_type` (
  ...
  `dash_type` varchar(255) DEFAULT NULL,
  `note` text DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_dashboard` (`dash_type`),
  CONSTRAINT `FK_dashboard` FOREIGN KEY (`dash_type`) REFERENCES `dashboards` (`name`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
