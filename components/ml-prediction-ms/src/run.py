...
class Parser: 
	def Data_Parser(self, data_val,data_dic,features): 
		"""  
		Perform Data Parser   
...
class Prediction: 
	...
	def Logic(self, Prev_Thresh,Current_Thresh):   
		...
		try: 
			...
			for cell_instance in range(len(Prev_Thresh)): 
			...
			for i in range(len(Prev_Thresh)):
	...
	def Final_Post_Method(Predicted_Results, Previous_Results, slices, server_name): 
		"""  
		...
		try: 
			...
			for key in Predicted_Results.keys(): 
				...
				temp_dict_2['configData']['predictedMaxNumberofConns'] = Predicted_Results[key] 
				now = datetime.now() 
...
class Controller: 
	...
	def simulatedTestDataToReplaceTopic(self):
		...
		try:
			...
			Final_Post_Method(predicted, configured, slice_name, server_name)  
			results=pd.DataFrame(data_dic) 
			logger.info("Predicted Results:"+results)   
			#how long this file will be written
			results.to_csv("predicted.csv", index=0)
		except Exception as e:
	...
	def PreprocessAndPredict(self,pm_data):
		...
		try:
			for i in range(len(pm_data)): 
				sub_data=json.loads(pm_data[i]) 
				sub_data = sub_data['event']['perf3gppFields']['measDataCollection']['measInfoList'][0] 
				server_name = eval(doc[x][0])['event']['perf3gppFields']['measDataCollection']['measuredEntityDn']  
				...
				self.data_dic= self.Parser_Object.Data_Parser(data_val,self.data_dic,features) 
	...
	def Execute(self): 
		try: 
			while True:
				...
				#TODO adjust - Wait for data generation  
				time.sleep(15) 

				self.StopDataGeneration()			
				#TODO adjust - Wait for data retrival  
				time.sleep(15) 
