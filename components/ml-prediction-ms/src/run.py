...
class Prediction:
    ...
    def Dmaap_url_Exist(self):
        try:
            ...
            response = requests.post(self.Config_Object.get_pathToDmapTopic(), json_object, verify=False, headers={"Content-Type":"application/json"}, timeout=3)
            data = response.json()
        except Exception as e:
            logger.error("traceID-%s Error Dmaap url Exist f:\n%s",str(uuid.uuid4())[:8], e)
        except requests.Timeout as error:
            logger.critical("traceId-%s Timeout from post to Dmap topic :\n%s",str(uuid.uuid4())[:8], error)
        return data
...
class Controller:
    ...
    def  GetData(self):
        ...
        pm_data = []
        try:
            response = requests.get(self.Config_Object.get_pathToGetData(), verify=False,timeout=3)
    ...
    def simulatedTestDataToReplaceTopic(self):
        ...
        try:
            ...
            for i  in range(len_df-1):
                ...
                ret = self.Predict_Object.Final_Post_Method(predicted, configured, slice_name, 'cucpserver1')  #hardcoding the server name
                if(ret == false): 
                    status = False
                    break 
