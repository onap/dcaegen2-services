    ...
    def Data_Parser(self, data_val,data_dic,features, slice_name):
        ...
        try:
            ...
            for i in range(len_data_val):
                ...
                response = requests.get(self.Config_Object.get_pathToGetConfigData() + cell_id + '/snssai/'+ slice_name)
...
class Prediction:
    ...
    def __init__(self, IsUnitetestPath):
        self.Config_Object=Config()
        self.IsUnitetestPath = IsUnitetestPath

    def Dmaap_url_Exist(self):
        try:
            Post_Message ={}
            json_object = json.dumps(Post_Message, indent = 4)
            response = requests.post(self.Config_Object.get_pathToDmapTopic(), json_object, verify=False, headers={"Content-Type":"application/json"})
            data = response.json()
        except Exception as e:
            logger.error("traceID-%s Error Dmaap url Exist f:\n%s",str(uuid.uuid4())[:8], e)
        return data
...
class Controller:
    ...
    def __init__(self, IsUnitetestPath):
        ...
        self.Predict_Object=Prediction(IsUnitetestPath)
        self.Config_Object=Config()
        self.IsUnitetestPath = IsUnitetestPath
...
if __name__ == "__main__":
    try:
        ...
        time.sleep(30)
        logger.info("traceId-%s : Start Prediction",str(uuid.uuid4())[:8])
        Controller_Object = Controller(False)
