...
def Parser(cells_data_list,data_dic,features): 
	"""
	Perform Data Parser 
	...
	try:
		...
		for i in range(len(cells_data_list)): 
			cell_id =cells_data_list[i]['measObjInstId'] 
			...
			print(config_data) 
			results= cells_data_list[i]['measResults'] 
			...
			for j in range(3,5): 
				...
				if key not in data_dic: 
					...
					change= data_dic[cell_id+'_SM.PDUSessionSetupFail.0'][-1]/data_dic[cell_id+'_SM.PDUSessionSetupReq.01'][-1] - data_dic[cell_id+'_SM.PDUSessionSetupFail.0'][-2]/data_dic[cell_id+'_SM.PDUSessionSetupReq.01'][-2] 
...
def GetSlicesCellsData():
	...
	#Wait for 2 hrs for data synchup
	time.sleep(3600*2) 
	...
	try:
		...
		while True: 
			...
			for x in range(len(data)): 
				...
				data_dic= Parser(cells_data_list,data_dic,features)        #calling the parser method 
		...
		test_data= result.iloc[91000:,:] 
		train_data=result.iloc[:91000,:] 
		train_data.to_excel('Train_Data.xlsx', index=0) 
		test_data.to_excel('Test_Data.xlsx',index=0) 
	except Exception as e:		
...
def Train(): 
	...
	try:
		...
		train = value[:72800, :]   #set the range of train, validation and test data 
		val= value[72800:81900,:] 
		test = value[81900:, :] 
