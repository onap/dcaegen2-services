...
class ControllerTestCase(unittest.TestCase):
    ...
    def test_GetData(self, mock_get):
        ...
        ctl = Controller(False)
        json_data = ctl.GetData()

        self.assertEqual(json_data, {"key1": "value1"})

        # We can even assert that our mocked method was called with the right parameters
        self.assertIn(mock.call(ctl.Config_Object.get_pathToGetData(), verify=False), mock_get.call_args_list)
...
class PredictionTestCase(unittest.TestCase):
    ...
    def test_Dmaap_url_Exist(self, mock_post):
        ...
        # We can even assert that our mocked method was called with the right parameters
        self.assertIn(mock.call(pred.Config_Object.get_pathToDmapTopic(), json_object, verify=False, headers={"Content-Type":"application/json"}), mock_post.call_args_list)
