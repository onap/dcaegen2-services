...
public class SliceUtilization {
    ...
    private static final Integer TORAL_PRB = 132;
    ...
    */
    ...
    protected AggregatedConfig calculateSliceUtilization(List<JSONObject> pmDataList) {
        ...
        while (j <= J) {
            tempdl += ((Math.pow(V, j) * Math.pow(QM, j) * Math.pow(F, j) * R_MAX * (TORAL_PRB - averageDLPrb) * 12
                            * (1 - Math.pow(OH_Dl, j))) / TS) * Math.pow(10, -6);
            tempul += ((Math.pow(V, j) * Math.pow(QM, j) * Math.pow(F, j) * R_MAX * (TORAL_PRB - averageULPrb) * 12
