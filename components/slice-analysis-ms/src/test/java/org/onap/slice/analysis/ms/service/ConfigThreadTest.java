...
public class ConfigThreadTest {
    ...
    public void run1Test() {
        ...
        doThrow(new RuntimeException()).when(configPolicy).getConfig();
        configThread.run();
    ...
    public void run2Test() {
        ...
        when(config.containsKey("cllId")).thenReturn(true);

        when(config.containsKey("cllId")).thenReturn(true).thenThrow(new RuntimeException());
        configThread.run();
        Mockito.verify(configThread, Mockito.atLeastOnce()).run();
    }
    ...
    public void run3Test() {
        ...
        Map<String, Object> config = mock(HashMap.class);
        when(configPolicy.getConfig()).thenReturn(config);
        when(config.containsKey("closedLoopStatus")).thenReturn(true);

        when(config.containsKey("closedLoopStatus")).thenReturn(true).thenThrow(new RuntimeException());
        configThread.run();
        Mockito.verify(configThread, Mockito.atLeastOnce()).run();
    }
    ...
    public void run4Test() {
        ...
        Map<String, Object> config = mock(HashMap.class);
        when(configPolicy.getConfig()).thenReturn(config);
        when(config.containsKey("originalBw")).thenReturn(true);

        when(config.containsKey("originalBw")).thenReturn(true).thenThrow(new RuntimeException());
        configThread.run();
        Mockito.verify(configThread, Mockito.atLeastOnce()).run();
    }
